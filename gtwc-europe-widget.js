...
let w;try{const x=await load(),bg=await hero();const __mhFam=config.widgetFamily||'medium';w=__mhFam==='small'?small(x.d,x.cached,bg):__mhFam==='large'?large(x.d,x.cached,bg):medium(x.d,x.cached,bg)}catch(e){w=base(null);w.setPadding(12,12,12,12);pill(w,'GTWC EUROPE',true);w.addSpacer(8);T(w,'データ取得失敗',18,col(C.text),'heavy');w.addSpacer(3);T(w,'通信回復後に再取得します',9.5,col(C.warn),'semibold')}
