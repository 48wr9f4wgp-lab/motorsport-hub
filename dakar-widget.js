// Motorsport Hub v9.5.0-hardening — DAKAR dedicated rally-raid module
...
(async()=>{
const V='9.5.0-hardening',K='dakar',SEASON=2027,CACHE_SCHEMA=1,CACHE_MAX_AGE=7*86400000;
...
const fm=FileManager.local(),DOC=fm.documentsDirectory(),CACHE=fm.joinPath(DOC,'motorsport-data-v950-dakar.json');
...
// Exact Commons file page verified: Alexander-93 / CC BY-SA 4.0 / original 5378×3588.
const HERO_URLS=[
...
async function txt(url){const r=new Request(url);r.timeoutInterval=10;r.headers={'User-Agent':'Mozilla/5.0 MotorsportHub/9.5','Cache-Control':'no-cache'};return await r.loadString()}
...
function cover(img,W,H,focus=.55,shift=0){const iw=img.size.width||1,ih=img.size.height||1,s=Math.max(W/iw,H/ih),dw=iw*s,dh=ih*s;return new Rect(-(dw-W)*focus+shift,-(dh-H)*.5,dw,dh)}
async function hero(){const small=(config.widgetFamily||'medium')==='small',p=fm.joinPath(DOC,`motorsport-hero-v950-${small?'small':'medium'}-dakar.jpg`);if(fm.fileExists(p)){try{return fm.readImage(p)}catch(_){} }try{let img=null;for(const u of HERO_URLS){try{const r=new Request(`${u}&v=950`);r.timeoutInterval=10;r.headers={'User-Agent':'Mozilla/5.0','Cache-Control':'no-cache'};img=await r.loadImage();if(img)break}catch(_){}}if(!img)return null;const W=small?720:1380,H=small?720:640,ctx=new DrawContext();ctx.size=new Size(W,H);ctx.opaque=true;ctx.respectScreenScale=false;ctx.setFillColor(col(C.bg));ctx.fillRect(new Rect(0,0,W,H));ctx.drawImageInRect(img,cover(img,W,H,.58,small?18:70));ctx.setFillColor(col('#060503',.12));ctx.fillRect(new Rect(0,0,W,H));for(let x=0;x<W;x+=3){const t=x/(W-1),a=.84*(1-smooth(t))+.055;ctx.setFillColor(col('#050403',a));ctx.fillRect(new Rect(x,0,4,H))}const rs=W*.74;for(let x=rs;x<W;x+=3){const t=(x-rs)/(W-rs),a=.07+.42*smooth(t);ctx.setFillColor(col('#050403',a));ctx.fillRect(new Rect(x,0,4,H))}const bs=H*.68,bh=H-bs;for(let i=0;i<56;i++){const y=bs+i*(bh/56),t=i/55,a=.02+.24*t*t;ctx.setFillColor(col('#040302',a));ctx.fillRect(new Rect(0,y,W,bh/56+1))}ctx.setFillColor(col(S.accent,.94));ctx.fillRect(new Rect(0,0,W,5));const out=ctx.getImage();try{fm.writeImage(p,out)}catch(_){}return out}catch(_){return null}}
...
function base(bg){const w=new ListWidget();if(bg)w.backgroundImage=bg;else{const g=new LinearGradient();g.colors=[col(S.accent,.18),col(C.bg)];g.locations=[0,1];w.backgroundGradient=g}w.url=S.url;return w}
