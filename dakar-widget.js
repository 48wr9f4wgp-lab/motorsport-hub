...
function validGap(g,pos){const x=String(g||'').trim();return Number(pos)===1?x==='—'||x==='+0:00':/^\+\d{1,3}:\d{2}(?::\d{2})?$/.test(x)}
function validRanking(a){return Array.isArray(a)&&a.length>=3&&a.slice(0,5).every(r=>r&&Number(r.pos)>=1&&String(r.name||'').trim()&&validGap(r.gap,r.pos))}
...
function timeSeconds(s){const m=String(s||'').match(/(?:(\d+)\s*h)?\s*(\d+)'\s*(\d+)''/);return m?Number(m[1]||0)*3600+Number(m[2])*60+Number(m[3]):NaN}
function gapText(sec){const n=Math.max(0,Math.round(Number(sec)||0)),h=Math.floor(n/3600),m=Math.floor((n%3600)/60),ss=n%60,p=v=>String(v).padStart(2,'0');return h?'+'+h+':'+p(m)+':'+p(ss):'+'+m+':'+p(ss)}
function parseRanking(h){
 ...
 const lead=timeSeconds(u[0]?.time);if(Number.isFinite(lead)){for(const r of u){if(Number(r.pos)===1){r.gap='—';continue}if(validGap(r.gap,r.pos))continue;const t=timeSeconds(r.time);if(Number.isFinite(t)&&t>=lead)r.gap=gapText(t-lead)}}
 return u
}
async function update(d,ctx=rankingContext()){const h=await txt(ctx.source),r=parseRanking(h);if(!validRanking(r))throw Error('DAKAR CAR standings');return nextStage({...d,ranking:r,rankingSeason:ctx.season,rankingLabel:ctx.label})}
...
function largeStanding(st,r){const row=st.addStack();row.layoutHorizontally();row.centerAlignContent();const ps=row.addStack();ps.size=new Size(18,0);T(ps,r.pos,10.5,col(S.accent),'heavy');row.addSpacer(4);T(row,r.name,11.2,col(C.text),'semibold');row.addSpacer();T(row,r.gap||'—',10,col(C.text),'heavy')}
function large(d,cached,bg){const w=base(bg),ci=countdown(d),meta=largeCalendar(d);w.setPadding(12,14,12,14);const top=w.addStack();top.layoutHorizontally();top.centerAlignContent();pill(top,'DAKAR',true);top.addSpacer(7);T(top,d.lifecycle==='SEASON_ENDED'||d.seasonEnded?'シーズン終了':'次戦',8.5,col(C.dim),'bold');top.addSpacer();if(cached)T(top,'• 更新待ち',7.2,col(C.warn,.82),'semibold');w.addSpacer(5);T(w,stageLabel(d),26,col(C.text),'heavy',1);w.addSpacer(2);const info=w.addStack();info.layoutHorizontally();info.centerAlignContent();T(info,`${d.dateLabel} · SS ${d.special}km`,11.5,col(C.muted),'semibold');info.addSpacer(6);T(info,`｜ ${routeLabel(d)}`,10.2,col(C.dim),'semibold',1);info.addSpacer();const cp=info.addStack();cp.backgroundColor=ci.live?col(C.good,.28):col('#000000',.50);cp.cornerRadius=10;cp.setPadding(4,8,4,8);T(cp,ci.label,17.2,ci.live?col(C.good):col(C.text),'heavy');w.addSpacer(8);const hh=w.addStack();T(hh,`総合 CAR · ${d.rankingLabel||'OVERALL'}`,8.7,col(C.muted),'bold');hh.addSpacer();T(hh,'GAP',7.8,col(C.text,.96),'bold');w.addSpacer(3);for(const r of(d.ranking||[]).slice(0,3)){const row=w.addStack();row.layoutHorizontally();row.centerAlignContent();const ps=row.addStack();ps.size=new Size(20,0);T(ps,r.pos,12,col(S.accent),'heavy');row.addSpacer(4);T(row,r.name,13.2,col(C.text),'semibold');row.addSpacer();const pts=row.addStack();pts.backgroundColor=col('#000000',.46);pts.cornerRadius=7;pts.setPadding(1,6,1,6);T(pts,r.gap||'—',11.2,col(C.text),'heavy');const sr=w.addStack();sr.addSpacer(24);T(sr,sub(r),8.8,col(C.dim),'semibold');w.addSpacer(2)}w.addSpacer();/* MH_LARGE_LOWER_ANCHOR_FLEX=1 */const lower=w.addStack();lower.layoutHorizontally();const more=lower.addStack();more.layoutVertically();more.backgroundColor=col('#000000',.42);more.cornerRadius=12;more.setPadding(8,10,8,10);T(more,'MORE STANDINGS',7.8,col(C.muted),'bold');more.addSpacer(4);const extras=(d.ranking||[]).slice(3,5);if(extras.length)for(const r of extras){largeStanding(more,r);more.addSpacer(3)}else T(more,'—',12,col(C.dim),'semibold');lower.addSpacer(8);const season=lower.addStack();season.layoutVertically();season.backgroundColor=col('#000000',.42);season.cornerRadius=12;season.setPadding(8,10,8,10);T(season,'STAGE',7.8,col(C.muted),'bold');season.addSpacer(3);T(season,d.stageId==='P'?'PROLOGUE':`${d.stageId} / 13`,12.2,col(C.text),'heavy');season.addSpacer(5);T(season,'PREVIOUS',7.2,col(C.dim),'bold');T(season,largeDisplayEvent(meta.previous),9.6,col(C.muted),'semibold',1);season.addSpacer(4);T(season,'NEXT',7.2,col(C.dim),'bold');T(season,largeDisplayEvent(meta.following),9.6,col(C.muted),'semibold',1);w.refreshAfterDate=new Date(Date.now()+20*60000);return w}
function small(d,cached,bg){const w=base(bg),ci=countdown(d);w.setPadding(10,11,9,11);const top=w.addStack();top.layoutHorizontally();top.centerAlignContent();pill(top,'DAKAR',true);top.addSpacer(4);T(top,d.seasonEnded?'FINISH':'次ステージ',7.1,col(C.dim),'semibold');top.addSpacer();const cp=top.addStack();cp.backgroundColor=ci.live?col(C.good,.30):col('#000000',.52);cp.cornerRadius=8;cp.setPadding(2,6,2,6);T(cp,ci.label,11.5,ci.live?col(C.good):col(C.text),'heavy');w.addSpacer(7);T(w,stageLabel(d),18.2,col(C.text),'heavy',2);w.addSpacer(3);T(w,`${d.dateLabel} · SS ${d.special}km`,9.7,col(C.muted),'semibold',1);w.addSpacer();const foot=w.addStack();const loc=foot.addStack();loc.backgroundColor=col('#000000',.48);loc.cornerRadius=7;loc.setPadding(2,5,2,5);T(loc,routeLabel(d),7.9,col(C.muted),'semibold',1);foot.addSpacer();if(cached)T(foot,'•',6.5,col(C.warn,.70),'semibold');w.refreshAfterDate=new Date(Date.now()+20*60000);return w}

let w;try{const x=await load(),bg=await hero();const __mhFam=config.widgetFamily||'medium';w=__mhFam==='small'?small(x.d,x.cached,bg):__mhFam==='large'?large(x.d,x.cached,bg):medium(x.d,x.cached,bg)}catch(_){w=base(null);w.setPadding(12,12,12,12);pill(w,'DAKAR',true);w.addSpacer(8);T(w,'データ取得失敗',18,col(C.text),'heavy');w.addSpacer(3);T(w,'通信回復後に再取得します',9.5,col(C.warn),'semibold')}
