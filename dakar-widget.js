// Motorsport Hub v9.5.1-hardening — DAKAR dedicated rally-raid module
...
// Tap Action v1: tap widget to cycle three persisted Hero framing presets; Medium DAKAR badge opens official site.
(async()=>{
const V='9.5.1-hardening',K='dakar',SEASON=2027,CACHE_SCHEMA=1,CACHE_MAX_AGE=7*86400000;
...
function readUI(){try{if(!fm.fileExists(UI_STATE))return{heroVariant:0};const p=JSON.parse(fm.readString(UI_STATE)),i=Number(p?.heroVariant);return{heroVariant:Number.isInteger(i)&&i>=0&&i<HERO_FRAMES.length?i:0}}catch(_){return{heroVariant:0}}}
...
if(ACTION==='cycleHero'){UI.heroVariant=(UI.heroVariant+1)%HERO_FRAMES.length;saveUI(UI)}
function tapURL(){try{if(typeof URLScheme!=='undefined'&&URLScheme.forRunningScript){const b=URLScheme.forRunningScript(),sep=String(b).includes('?')?'&':'?';return`${b}${sep}mhCategory=DAKAR&mhAction=cycleHero`}}catch(_){}return S.url}
...
async function txt(url){const r=new Request(url);r.timeoutInterval=10;r.headers={'User-Agent':'Mozilla/5.0 MotorsportHub/9.5.1','Cache-Control':'no-cache'};return await r.loadString()}
