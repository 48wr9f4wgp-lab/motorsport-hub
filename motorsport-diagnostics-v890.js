// Motorsport Hub v9.2.0 — QA diagnostics
// Manual diagnostics: checks the ten live data routes without changing any widget cache/state.
(async()=>{
...
const run=async t=>{const s=Date.now();try{const r=new Request(t.url);r.timeoutInterval=10;r.headers={'User-Agent':'Mozilla/5.0 MotorsportHubQA/9.2','Cache-Control':'no-cache'};const data=t.kind==='json'?await r.loadJSON():await r.loadString();const ok=!!t.check(data);return{name:t.name,ok,ms:Date.now()-s,msg:ok?'LIVE':'PARSE'}}catch(e){return{name:t.name,ok:false,ms:Date.now()-s,msg:'NET'}}};
const results=await Promise.all(tests.map(run));
const all=results.every(x=>x.ok),count=results.filter(x=>x.ok).length,w=new ListWidget();w.backgroundColor=new Color(C.bg);w.setPadding(9,14,8,14);
const title=w.addText('Motorsport Hub  QA');title.font=Font.heavySystemFont(16);title.textColor=new Color(C.text);
const sub=w.addText(all?'10/10 LIVE — データ経路OK':`${count}/10 LIVE — 要確認`);sub.font=Font.semiboldSystemFont(9.5);sub.textColor=new Color(all?C.ok:C.warn);w.addSpacer(4);
for(const x of results){const row=w.addStack();row.layoutHorizontally();row.centerAlignContent();const a=row.addText('●');a.font=Font.boldSystemFont(9.1);a.textColor=new Color(x.ok?C.ok:C.bad);row.addSpacer(5);const n=row.addText(x.name);n.font=Font.semiboldSystemFont(9.2);n.textColor=new Color(C.text);n.lineLimit=1;n.minimumScaleFactor=.68;row.addSpacer();const st=row.addText(`${x.msg}  ${x.ms}ms`);st.font=Font.systemFont(8);st.textColor=new Color(x.ok?C.muted:C.bad);w.addSpacer(1)}
w.addSpacer(1);const foot=w.addText('※ 通信/一次パース診断。新規カテゴリはSmall/Medium実機QAで最終確認。');foot.font=Font.systemFont(7.3);foot.textColor=new Color(C.muted);foot.lineLimit=2;w.refreshAfterDate=new Date(Date.now()+10*60000);
