// Motorsport Hub v9.5.0-hardening — QA diagnostics
...
const tests=[
 ...
 {name:'DAKAR',requests:[
  ...
  {kind:'text',url:'https://www.dakar.com/fr/webview/rankings/stage-13/auto?year=2026',check:x=>/NASSER AL-ATTIYAH/i.test(x)&&/NANI ROMA/i.test(x)&&/MATTIAS EKSTR[ÖO]M/i.test(x)}
 ]}
];
async function request(q){const r=new Request(q.url);r.timeoutInterval=10;r.headers={'User-Agent':'Mozilla/5.0 MotorsportHubQA/9.5.0','Cache-Control':'no-cache'};const data=q.kind==='json'?await r.loadJSON():await r.loadString();return!!q.check(data)}
