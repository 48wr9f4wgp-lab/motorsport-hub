...
const patcher=fs.readFileSync(path.join(root,'tools/apply-hero-channel-runtime.py'),'utf8');
...
assert(router.includes('HERO_CHANNEL_TTL=15*60000'),'Hero manifest cache must refresh within 15 minutes');
assert(patcher.includes('HERO_CHANNEL_TTL=15*60000'),'Hero runtime patcher must preserve the 15-minute manifest TTL');
...
assert(patcher.includes("if(cat==='QA')return null"),'Hero runtime patcher must keep Dakar inside the shared Hero channel');
assert(!patcher.includes("if(cat==='DAKAR'||cat==='QA')return null"),'Hero runtime patcher must not regress Dakar exclusion');
