...
assert.equal(descriptor.sourceRef,sourceRef);
assert.equal(descriptor.routerSchema,5);
assert.equal(Object.keys(descriptor.files).length,registry.categories.length+1,'release descriptor must include 11 category modules + QA');
const router=fs.readFileSync(path.join(root,'motorsport-hub.js'),'utf8');
assert.equal(descriptor.router.sha256,sha(router));
assert.equal(descriptor.router.bytes,Buffer.byteLength(router,'utf8'));
...
assert(generated.includes(sourceRef),'generated loader must hard-pin immutable commit');
assert(generated.includes(descriptor.router.sha256),'generated loader must embed Router SHA-256');
assert(generated.includes('__MH_RELEASE_INTEGRITY'),'generated loader must pass release integrity to Router');
assert(!generated.includes('/main/motorsport-hub.js'),'generated loader must never fetch mutable main');

const manifest='F1,WEC,WRC,SUPERGT,MOTOGP,FDJ,D1GP,SUPERFORMULA,INDYCAR,NASCAR,GTWCEU,QA';
...
async function run({response=synthetic,seed={}}={}){
 ...
 const ctx={config:{runsInWidget:true,widgetFamily:'small'},FileManager:{local:()=>fm},Request,ListWidget:CtxListWidget,Color,Font,Date,Math,JSON,String,Number,Array,Object,RegExp,Error,Promise,Script:{complete(){complete++},setWidget(){setWidget++}}};ctx.globalThis=ctx;vm.createContext(ctx);await vm.runInContext(loader,ctx,{timeout:5000});return{ctx,sink,fm,requests,complete,setWidget};
}

{
 const r=await run();
 assert.equal(r.ctx.__SYNTHETIC_ROUTER_EXECUTED,true,'valid immutable Router must execute');
 assert.equal(r.complete,1);assert.equal(r.setWidget,0);assert.equal(r.requests,1);
 const lkg='/docs/motorsport-hub-router-v6-mh-synthetic-lkg.js';assert.equal(r.fm.files.get(lkg),synthetic,'successful immutable Router must promote to LKG');
}
{
 const r=await run({response:synthetic+'// tamper'});
 assert.equal(r.ctx.__SYNTHETIC_ROUTER_EXECUTED,undefined,'tampered Router must not execute');assert.equal(r.setWidget,1);assert.equal(r.complete,1);assert(r.sink.some(x=>x.includes('固定Releaseを取得できません')));
}
{
 const lkg='/docs/motorsport-hub-router-v6-mh-synthetic-lkg.js';
 const r=await run({response:new Error('offline'),seed:{[lkg]:synthetic}});
 assert.equal(r.ctx.__SYNTHETIC_ROUTER_EXECUTED,true,'offline verified LKG must execute');assert.equal(r.complete,1);
}
