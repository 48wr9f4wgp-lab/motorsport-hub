...
assert(generated.includes('motorsport-hub-observability-v1.json'));assert(generated.includes("obs('CANDIDATE',true)"));assert(generated.includes("obs('LKG',true)"));assert(!generated.includes('deviceId'));assert(!generated.includes('email'));assert(!generated.includes('latitude'));assert(!generated.includes('longitude'));
...
async function run({response=synthetic,seed={},parameter='WRC'}={}){const sink=[],fm=fmWith(seed);let requests=0,complete=0,setWidget=0;class Request{constructor(url){this.url=url;this.headers={};requests++}async loadString(){if(response instanceof Error)throw response;return response}}const CtxListWidget=class extends ListWidget{constructor(){super(sink)}};const ctx={config:{runsInWidget:true,widgetFamily:'small'},args:{widgetParameter:parameter,queryParameters:{}},FileManager:{local:()=>fm},Request,ListWidget:CtxListWidget,Color,Font,Date,Math,JSON,String,Number,Array,Object,RegExp,Error,Promise,Script:{complete(){complete++},setWidget(){setWidget++}}};ctx.globalThis=ctx;vm.createContext(ctx);await vm.runInContext(loader,ctx,{timeout:5000});return{ctx,sink,fm,requests,complete,setWidget};}
const obsPath='/docs/motorsport-hub-observability-v1.json';

{const r=await run();assert.equal(r.ctx.__SYNTHETIC_ROUTER_EXECUTED,true);assert.equal(r.complete,1);assert.equal(r.setWidget,0);assert.equal(r.requests,1);const lkg='/docs/motorsport-hub-router-v6-mh-synthetic-lkg.js';assert.equal(r.fm.files.get(lkg),synthetic);const o=JSON.parse(r.fm.files.get(obsPath));assert.equal(o.schemaVersion,1);assert.equal(o.events.length,1);assert.equal(o.events[0].category,'WRC');assert.equal(o.events[0].family,'small');assert.equal(o.events[0].path,'CANDIDATE');assert.equal(o.events[0].ok,true);assert(Number.isFinite(o.events[0].ms));assert(!('deviceId' in o.events[0]));}
{const r=await run({response:synthetic+'// tamper'});assert.equal(r.ctx.__SYNTHETIC_ROUTER_EXECUTED,undefined);assert.equal(r.setWidget,1);assert.equal(r.complete,1);assert(r.sink.some(x=>x.includes('固定Releaseを取得できません')));const o=JSON.parse(r.fm.files.get(obsPath));assert.equal(o.events.at(-1).path,'FAIL');assert.equal(o.events.at(-1).ok,false)}
{const lkg='/docs/motorsport-hub-router-v6-mh-synthetic-lkg.js';const r=await run({response:new Error('offline'),seed:{[lkg]:synthetic},parameter:'SUPER GT'});assert.equal(r.ctx.__SYNTHETIC_ROUTER_EXECUTED,true);assert.equal(r.complete,1);const o=JSON.parse(r.fm.files.get(obsPath));assert.equal(o.events.at(-1).category,'SUPERGT');assert.equal(o.events.at(-1).path,'LKG');assert.equal(o.events.at(-1).ok,true)}
{const many={schemaVersion:1,events:Array.from({length:200},(_,i)=>({ts:String(i),releaseId:'old',category:'F1',family:'small',path:'CANDIDATE',ok:true,ms:1}))};const r=await run({seed:{[obsPath]:JSON.stringify(many)}});const o=JSON.parse(r.fm.files.get(obsPath));assert.equal(o.events.length,200);assert.equal(o.events.at(-1).releaseId,'mh-synthetic')}
