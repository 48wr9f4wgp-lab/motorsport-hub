...
class Stack {
  ...
  addText(v){return new Text(v,this.s)} addSpacer(){} addStack(){return new Stack(this.s)}
...
async function render(category,family){
  const record=registry.categories.find(x=>x.id===category);
  assert(record,`registry missing ${category}`);
  ...
  class Request {
    ...
    async loadString(){if(this.url.includes(record.module)){repoRequests++;return moduleSource}throw Error('offline data source')}
    async loadJSON(){throw Error('offline data source')}
    async loadImage(){throw Error('hero cache should be used')}
  }
  const CtxListWidget=class extends ListWidget{constructor(){super(sink)}};
  const ctx={
    args:{widgetParameter:category},config:{runsInWidget:true,widgetFamily:family},
  ...
  assert.equal(repoRequests,1,`${category}/${family}: must fetch exactly one category module`);
  assert.equal(setWidget,1,`${category}/${family}: setWidget count`);
  assert.equal(complete,1,`${category}/${family}: Script.complete count`);
  const text=sink.join(' | ');
  assert(!/データ取得失敗|安全に実行できません|Widget Parameterが不正/.test(text),`${category}/${family}: rendered error state: ${text}`);
  assert(expected[category]?.test(text),`${category}/${family}: expected event identity missing: ${text}`);
  if(family==='medium')assert(/PTS|ポイント/.test(text),`${category}/medium: standings points header missing`);
  return text;
}

for(const c of registry.categories){
  for(const family of ['small','medium'])await render(c.id,family);
}

console.log('Motorsport Hub 22-case render smoke gate: PASS');
