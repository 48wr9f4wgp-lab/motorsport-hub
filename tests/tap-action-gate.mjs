...
for(const token of ['Tap Action v1','motorsport-ui-v1-dakar.json','HERO_FRAMES','mhAction=cycleHero','URLScheme.forRunningScript','heroVariant'])assert(dakar.includes(token),`Dakar tap invariant missing: ${token}`);
...
async function runTap(files){
 ...
 const fm={
  ...
  fileExists:p=>p.includes('motorsport-hero-v951-')||files.has(p),
