...
for(const p of ['README.md','INSTALL.md','SUPPORT.md','PRIVACY.md','GA_READINESS.md','GA_LAUNCH_CHECKLIST.md','LICENSE_DECISION.md','CHANGELOG.md','ATTRIBUTION.md','.github/ISSUE_TEMPLATE/bug_report.yml']){
...
for(const p of ['INSTALL.md','SUPPORT.md','PRIVACY.md','GA_READINESS.md','GA_LAUNCH_CHECKLIST.md','LICENSE_DECISION.md','ATTRIBUTION.md'])assert(readme.includes(p),`README must link ${p}`);
...
assert(install.includes('LICENSE_DECISION.md'),'install guide must disclose unresolved software-license decision');
...
assert(attribution.includes('Current live-pool audit snapshot'),'attribution must retain current live-pool review evidence');
assert(attribution.includes('implementation-ready'),'attribution must retain final publication gate wording');

const licenseDecision=read('LICENSE_DECISION.md');
assert(licenseDecision.includes('Mozilla Public License 2.0 (MPL-2.0)'),'license decision must record the recommended license');
assert(licenseDecision.includes('scriptable/club-pulse*'),'license decision must record the repository-scope blocker');
assert(licenseDecision.includes('does **not** grant a license'),'license decision must not itself license the software');
...
assert(ga.includes('MPL-2.0'),'GA readiness must record the narrowed license recommendation');
assert(ga.includes('Club Pulse'),'GA readiness must retain repository-scope risk');

const launch=read('GA_LAUNCH_CHECKLIST.md');
for(const token of ['Software license explicitly approved','ATTRIBUTION.md','Physical iPhone smoke','Owner explicitly authorizes broad GA'])assert(launch.includes(token),`GA launch checklist missing ${token}`);
