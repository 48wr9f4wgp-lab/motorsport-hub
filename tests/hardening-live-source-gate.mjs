...
assert(router.includes('// Motorsport Hub v9.5.3-hardening — direct category module router'),'hardening Router version drift');
...
assert(router.includes('?v=953&t=${Date.now()}-${Math.random()}'),'Router request cache buster drift');
assert(router.includes("'User-Agent':'MotorsportHubRouter/9.5.3-hardening'"),'Router user-agent drift');
