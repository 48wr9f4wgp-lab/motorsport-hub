// Motorsport Hub v8.7.0 — Universal Readability Pass
// Applies one contrast system across F1 / WEC / WRC / SUPER GT / MotoGP / FDJ.
(async()=>{
...
const cache=fm.joinPath(DOC,`motorsport-universal-source-v870-${key}.js`);
...
try{
  ...
  r.headers={'Cache-Control':'no-cache, no-store, max-age=0, must-revalidate','Pragma':'no-cache','Expires':'0','User-Agent':'MotorsportHub/8.7'};
...
if(!valid(code)){await fail('v8.7表示モジュールを取得できません。旧版へ戻さず再試行します。');return}
...
if(!isFDJ&&!isHQ){
  ...
  code=rep(code,'motorsport-hero-v840-small-','motorsport-hero-v870-small-');
  code=rep(code,'motorsport-hero-v832-','motorsport-hero-v870-medium-');
...
// ---------- FDJ ----------
if(isFDJ){
  code=rep(code,"const V='8.5.4'","const V='8.7.0'");
  code=code.replace(/const HERO_URLS=\[[\s\S]*?\];/,
    "const HERO_URLS=['https://commons.wikimedia.org/wiki/Special:Redirect/file/Nissan%20Silvia%20S14%20Drift.jpg?width=2048'];"
  );
  code=rep(code,'motorsport-hero-v854-','motorsport-hero-v870-');
...
if(isHQ){
  ...
  code=rep(code,'motorsport-hero-v860-','motorsport-hero-v870-');
  code=rep(code,'&v=860','&v=870');
...
try{await eval(code)}catch(e){await fail('v8.7可読性処理に失敗しました。数分後に再試行します。')}
finally{
