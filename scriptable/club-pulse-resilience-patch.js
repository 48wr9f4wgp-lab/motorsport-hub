...
buildHeaderSmall=function(w,d,img){
  ...
  h.addSpacer(5);heavy(h,club.jp,8.5,typeof CP_MU_IS==='function'&&CP_MU_IS()?CP_MU_THEME.ivory:'#FFFFFF');
  if(d.stale){
  ...
  h.addSpacer();let rk=heavy(h,d.rank!=null?`${d.rank}位`:'–',9.5,typeof CP_MU_IS==='function'&&CP_MU_IS()?CP_MU_THEME.ivory:'#FFFFFF');rk.rightAlignText()
};
...
errorWidget=function(msg){
  ...
  let h=w.addStack();h.layoutHorizontally();h.centerAlignContent();
  heavy(h,'Club Pulse',family==='small'?11:13,typeof CP_MU_IS==='function'&&CP_MU_IS()?CP_MU_THEME.ivory:'#FFFFFF');
  ...
  let main=heavy(w,'保存データがありません',family==='small'?10:13,typeof CP_MU_IS==='function'&&CP_MU_IS()?CP_MU_THEME.ivory:'#FFFFFF');main.centerAlignText();
