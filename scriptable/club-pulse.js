...
async function setupLiveToken(){
  if(getLiveToken()||!config.runsInApp)return getLiveToken();
  let a=new Alert();a.title='LIVE速報を有効化';a.message='API-Football の API-KEY を貼り付けてください。端末のKeychainにのみ保存します。';a.addTextField('API-KEY','');a.addAction('接続して保存');a.addCancelAction('後で');
  if(await a.present()===-1)return null;
  let t=a.textFieldValue(0).trim();if(!t)return null;
  try{
    let j=await liveApi(`/teams?search=${encodeURIComponent(club.liveSearch)}`,t),rows=j?.response||[];
    if(!rows.length)throw new Error('team lookup failed');
    Keychain.set(LIVE_TOKEN,t);
    let m=loadLiveMap(),q=club.liveSearch.toLowerCase(),best=rows.find(x=>String(x?.team?.name||'').toLowerCase()===q)||rows[0],id=best?.team?.id||null;if(id){m[club.id]=id;saveLiveMap(m)}
    let ok=new Alert();ok.title='LIVE速報 接続完了';ok.message='API-Footballの接続確認と保存が完了しました。今後は試合時間帯だけ自動でLIVEデータを確認します。';ok.addAction('OK');await ok.present();return t;
  }catch(e){let ng=new Alert();ng.title='LIVE速報 接続失敗';ng.message='API-KEYを確認してください。通常のClub Pulseはそのまま利用できます。';ng.addAction('OK');await ng.present();return null}
}

const teamName=t=>!t?'未定':JP[t.name]||JP[t.shortName]||t.shortName||t.name||t.tla||'未定';
const venueName=(v,h)=>VEN[v]||v||(h?club.venue:'アウェイ');
function compInfo(code,name){
  let c=String(code||'').toUpperCase(),n=String(name||'').trim(),x=n.toLowerCase();
  if(c==='PL'||x.includes('premier league'))return{full:'プレミアリーグ',short:'PL'};
  if(c==='PD'||x.includes('laliga')||x.includes('primera division'))return{full:'ラ・リーガ',short:'LaLiga'};
  if(c==='CL'||x.includes('champions league'))return{full:'チャンピオンズリーグ',short:'CL'};
  if(c==='EL'||(x.includes('europa league')&&!x.includes('conference')))return{full:'ヨーロッパリーグ',short:'EL'};
  if(c==='ECL'||x.includes('conference league'))return{full:'カンファレンスリーグ',short:'ECL'};
  if(c==='FAC'||x==='fa cup'||x.includes('fa cup'))return{full:'FAカップ',short:'FA杯'};
  if(x.includes('efl cup')||x.includes('league cup')||x.includes('carabao'))return{full:'EFLカップ',short:'EFL杯'};
  if(c==='CDR'||x.includes('copa del rey'))return{full:'国王杯',short:'国王杯'};
  if(x.includes('community shield'))return{full:'コミュニティ・シールド',short:'CS'};
  if(x.includes('super cup')||x.includes('supercopa'))return{full:n||'スーパーカップ',short:'SUPER'};
  if(x.includes('club world cup'))return{full:'クラブワールドカップ',short:'CWC'};
  let full=COMP[c]||n||'公式戦',short=c&&c.length<=6?c:(full.length<=7?full:full.slice(0,6));
  return{full,short};
}
...
function mapMatch(m){if(!m)return null;let h=m.homeTeam?.id===club.team,opp=h?m.awayTeam:m.homeTeam,own=h?m.homeTeam:m.awayTeam,s=scoreFor(m),ci=compInfo(m.competition?.code,m.competition?.name);return{id:m.id,status:m.status,minute:m.status==='PAUSED'?'HT':Number.isFinite(m.minute)?`${m.minute}${m.injuryTime?`+${m.injuryTime}`:''}'`:'試合中',homeAway:h?'HOME':'AWAY',kickoff:kickoff(m.utcDate),utcDate:m.utcDate,venue:venueName(m.venue,h),competition:ci.full,competitionShort:ci.short,competitionCode:m.competition?.code||'',opponentName:teamName(opp),opponentShort:opp?.tla||'OPP',opponentId:opp?.id||null,opponentCrest:opp?.crest||null,clubCrest:own?.crest||null,ourScore:s.ours,opponentScore:s.theirs,result:m.status==='FINISHED'?result(m):null}}
function mapData(mj,sj){let now=Date.now(),m=mj?.matches||[],fin=m.filter(x=>x.status==='FINISHED').sort((a,b)=>new Date(b.utcDate)-new Date(a.utcDate)),live=m.find(x=>['IN_PLAY','PAUSED','LIVE'].includes(x.status))||null,next=m.filter(x=>['SCHEDULED','TIMED'].includes(x.status)&&new Date(x.utcDate).getTime()>=now-144e5).sort((a,b)=>new Date(a.utcDate)-new Date(b.utcDate))[0]||null,last=fin[0]||null,post=last&&now-new Date(last.utcDate).getTime()<=POST?last:null,row=standing(sj),form=fin.slice(0,5).map(result);while(form.length<5)form.push('-');let L=mapMatch(live),N=mapMatch(next),P=mapMatch(post),crest=row?.team?.crest||L?.clubCrest||N?.clubCrest||P?.clubCrest||null;return{provider:PROVIDER,fetchedAt:Date.now(),mode:L?'LIVE':P?'POST':'NEXT',rank:row?.position??null,points:row?.points??null,form,clubCrest:crest,liveMatch:L,recentResult:P,nextMatch:N}}
...
function mapLiveFixture(f,teamId){if(!f)return null;let h=f?.teams?.home?.id===teamId,opp=h?f.teams.away:f.teams.home,own=h?f.teams.home:f.teams.away,st=f?.fixture?.status||{},elapsed=st.elapsed,ci=compInfo('',f?.league?.name);return{id:f?.fixture?.id||`live-${club.id}`,status:'IN_PLAY',minute:st.short==='HT'?'HT':Number.isFinite(elapsed)?`${elapsed}'`:'LIVE',homeAway:h?'HOME':'AWAY',kickoff:kickoff(f?.fixture?.date||new Date().toISOString()),utcDate:f?.fixture?.date||new Date().toISOString(),venue:venueName(f?.fixture?.venue?.name,h),competition:ci.full,competitionShort:ci.short,competitionCode:'',opponentName:teamName(opp),opponentShort:opp?.code||'OPP',opponentId:opp?.id||null,opponentCrest:opp?.logo||null,clubCrest:own?.logo||null,ourScore:h?f?.goals?.home:f?.goals?.away,opponentScore:h?f?.goals?.away:f?.goals?.home,result:null}}
async function applyLiveOverlay(d){let token=getLiveToken();if(!token||!shouldCheckLive(d))return{...d,liveProvider:token?'ready':'unconfigured'};try{let teamId=await resolveLiveTeamId(token);if(!teamId)return{...d,liveProvider:'team-unresolved'};let j=await liveApi(`/fixtures?live=all&team=${teamId}`,token),f=(j?.response||[])[0];if(!f)return{...d,liveProvider:'ready'};let m=mapLiveFixture(f,teamId);return{...d,mode:'LIVE',liveMatch:m,clubCrest:m.clubCrest||d.clubCrest,liveProvider:'apiFootball',liveFetchedAt:Date.now()}}catch(e){return{...d,liveProvider:'error',liveError:String(e)}}}
...
function chip(p,r,small=false,latest=false){let m={W:['勝','#63E283','#133922'],D:['分','#D2D2D7','#37373C'],L:['負','#FF6961','#4D1715'],'-':['–','#8E8E93','#25252A']}[r],s=p.addStack();s.setPadding(small?1:2,small?5:6,small?1:2,small?5:6);s.cornerRadius=7;s.backgroundColor=C(m[2]);if(latest){s.borderWidth=1;s.borderColor=C(club.a,.9)}text(s,m[0],small?6.5:7,true,1,m[1])}
...
function competitionColor(label){return label==='CL'?'#8396FF':label==='EL'?'#FF9D4D':label==='ECL'?'#61D8A3':(label==='FA杯'||label==='EFL杯'||label==='国王杯')?'#F2C85B':club.a}
function competitionPill(parent,m,small=false){let label=m?.competitionShort||'公式戦',col=competitionColor(label),p=parent.addStack();p.setPadding(small?1:2,small?4:6,small?1:2,small?4:6);p.cornerRadius=7;p.backgroundColor=C(col,.12);p.borderWidth=.7;p.borderColor=C(col,.55);text(p,label,small?5.8:6.8,true,.95,col);return p}
...
function buildFooterMedium(w,d){let f=w.addStack();f.layoutHorizontally();f.centerAlignContent();f.setPadding(2,8,2,8);f.cornerRadius=9;f.backgroundGradient=gradient([C('#121317'),C(club.s,.70),C(club.p,.28)],[0,.5,1]);text(f,'最新',6.2,true,.9,club.a);f.addSpacer(5);for(let i=0;i<d.form.length;i++){chip(f,d.form[i],true,i===0);if(i<d.form.length-1)f.addSpacer(3)}f.addSpacer()}
function buildMatchMedium(w,d,imgs){let m=d.mode==='LIVE'?d.liveMatch:d.mode==='POST'?d.recentResult:d.nextMatch,c=w.addStack();c.layoutVertically();c.setPadding(4,9,4,9);c.cornerRadius=16;c.backgroundGradient=cardBg(d.mode);if(!m){heavy(c,'試合データ未取得',11);return}let top=c.addStack();top.layoutHorizontally();top.centerAlignContent();text(top,statusTitle(d,m),8,true,1,statusAccent(d));top.addSpacer(5);competitionPill(top,m,false);top.addSpacer();if(d.mode==='LIVE')heavy(top,m.minute||'LIVE',10);else pill(top,sideTag(m));c.addSpacer(2);let outer=c.addStack();outer.layoutHorizontally();outer.centerAlignContent();outer.addSpacer();let row=outer.addStack();row.layoutHorizontally();row.centerAlignContent();renderTeamBlock(row,{img:imgs.club,name:club.jp,sub:club.short,fallback:club.badge,logoSize:56,nameSize:11.5,subSize:5.5,p1:club.p,p2:club.s,scale:CREST_SCALE[club.team]||1.08,nameGap:1,width:92});row.addSpacer(20);let mid=heavy(row,centerMainText(d,m),d.mode==='NEXT'?14:22);mid.centerAlignText();row.addSpacer(20);renderTeamBlock(row,{img:imgs.opp,name:m.opponentName,sub:m.opponentShort,fallback:m.opponentShort,logoSize:56,nameSize:11.5,subSize:5.5,p1:'#4A5568',p2:'#20242D',scale:CREST_SCALE[m.opponentId]||CREST_SCALE.opponent_default||1.08,nameGap:1,width:92});outer.addSpacer();c.addSpacer(2);let meta=c.addStack();meta.layoutHorizontally();meta.centerAlignContent();meta.addSpacer();let mt=semibold(meta,metaLine(d,m),8.7,.92);mt.centerAlignText();meta.addSpacer()}
...
function buildMatchSmall(w,d,imgs){let m=d.mode==='LIVE'?d.liveMatch:d.mode==='POST'?d.recentResult:d.nextMatch,c=w.addStack();c.layoutVertically();c.setPadding(5,5,5,5);c.cornerRadius=14;c.backgroundGradient=cardBg(d.mode);if(!m){heavy(c,'試合データ未取得',9);return}let top=c.addStack();top.layoutHorizontally();text(top,statusTitle(d,m),6.8,true,1,statusAccent(d));top.addSpacer(3);competitionPill(top,m,true);top.addSpacer();if(d.mode==='LIVE')heavy(top,m.minute||'LIVE',8.2);c.addSpacer(3);let outer=c.addStack();outer.layoutHorizontally();outer.centerAlignContent();outer.addSpacer();let row=outer.addStack();row.layoutHorizontally();row.centerAlignContent();renderTeamBlock(row,{img:imgs.club,name:club.jp,fallback:club.badge,logoSize:40,nameSize:6.9,p1:club.p,p2:club.s,scale:CREST_SCALE[club.team]||1.08,nameGap:1,width:48});row.addSpacer(4);let scoreBox=row.addStack();scoreBox.size=new Size(22,0);scoreBox.layoutHorizontally();scoreBox.addSpacer();let sc=heavy(scoreBox,centerMainText(d,m),d.mode==='NEXT'?13:16);sc.centerAlignText();scoreBox.addSpacer();row.addSpacer(4);renderTeamBlock(row,{img:imgs.opp,name:m.opponentName,fallback:m.opponentShort,logoSize:40,nameSize:6.9,p1:'#4A5568',p2:'#20242D',scale:CREST_SCALE[m.opponentId]||CREST_SCALE.opponent_default||1.08,nameGap:1,width:48});outer.addSpacer();c.addSpacer(3);let meta=c.addStack();meta.layoutHorizontally();meta.centerAlignContent();meta.addSpacer();let mt=semibold(meta,d.mode==='NEXT'?m.kickoff:m.competition,7.1,.88);mt.centerAlignText();meta.addSpacer()}
function buildFooterSmall(w,d){let f=w.addStack();f.layoutHorizontally();f.centerAlignContent();f.addSpacer();text(f,'最新',5.8,true,.9,club.a);f.addSpacer(4);for(let i=0;i<d.form.length;i++){chip(f,d.form[i],true,i===0);if(i<d.form.length-1)f.addSpacer(3)}f.addSpacer()}
...
if(config.runsInApp&&!getLiveToken())await setupLiveToken();
