...
let c=await core(),cr=await getRemote(CLUB_REGISTRY_PATCH,CRP,300,'clubs1'),x=await getRemote(PATCH,PP,300,'ui8'),y=await getRemote(COMP_PATCH,LP,300,'comp3'),z=await getRemote(THEME_PATCH,TP,300,'mutheme3'),tr=await getRemote(THEME_REGISTRY_PATCH,TRP,300,'themes1'),u=await getRemote(TOP_LAYOUT_PATCH,TLP,300,'toplayout2'),q=await getRemote(LIVE_CONTEXT_PATCH,LCP,250,'livectx1'),r=await getRemote(RESILIENCE_PATCH,RP,300,'resilience1');
...
/* Compatibility padding: keep this launcher above 1000 bytes because older installed recovery loaders reject downloaded runtime files shorter than 1000 bytes. Club Pulse now injects a declarative club registry before core parameter resolution, allowing new clubs such as Real Madrid to be selected without editing the frozen core. Runtime composition then loads stable UI, competition logos, the frozen Manchester United baseline theme, a shared multi-club theme registry for Barcelona and Real Madrid, top-layout v2, LIVE context, and resilience. New clubs should normally be added through the club registry plus shared theme tokens rather than by cloning renderer files. The Manchester United visual baseline remains pinned and unchanged. QA selections made while running inside Scriptable are persisted for 15 minutes in ClubPulseQAOverride_v1.json so Small and Medium home-screen widgets render the same QA state; choosing normal/auto clears the override and expired overrides self-delete. Resilience QA can force an offline-with-cache state without deleting data, or a no-cache error state while preserving the real cache. Competition identity colors and logos remain shared across clubs. */