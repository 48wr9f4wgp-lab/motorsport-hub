...
/* Compatibility padding: keep this launcher above 1000 bytes because older installed recovery loaders reject downloaded runtime files shorter than 1000 bytes. Club Pulse injects a declarative club registry before core parameter resolution, allowing Real Madrid and future clubs to be selected without editing the frozen core. Runtime composition loads stable UI, competition logos, the frozen Manchester United baseline theme, a shared multi-club theme registry for Barcelona and Real Madrid, top-layout v2, LIVE context, and resilience v2. New clubs should normally be added through the club registry plus shared theme tokens rather than by cloning renderer files. The Manchester United visual baseline remains pinned and unchanged. Resilience v2 also lets Small headers inherit the active shared club theme. QA selections made while running inside Scriptable are persisted for 15 minutes in ClubPulseQAOverride_v1.json so Small and Medium home-screen widgets render the same QA state; choosing normal/auto clears the override and expired overrides self-delete. Competition identity colors and logos remain shared across clubs. */