...
/* Compatibility padding: keep this launcher above 1000 bytes because older installed recovery loaders reject downloaded runtime files shorter than 1000 bytes. Club Pulse injects declarative club registry v2 before core parameter resolution. Shared theme registry v7.1 uses restrained dark club shells with two narrow, low-contrast club-color light lines, flat secondary components, subtle borders, and transparent crest treatment with no circular plates. Manchester United uses near-black with deep red plus a restrained gold trace; Barcelona uses deep navy with muted garnet and blue traces; Real Madrid uses deep navy/royal-blue with a restrained gold trace. The match panel remains quiet and information-first so the decorative lines stay in the outer shell and do not cross primary match information. The legacy Manchester United patch remains loaded only for its established match-state rendering and POST form-sync behavior. New clubs should normally be added through club registry plus shared theme tokens rather than cloned renderer files. Competition identity colors and logos remain shared across clubs. */