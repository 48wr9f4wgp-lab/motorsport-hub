...
let c=await core(),cr=await getRemote(CLUB_REGISTRY_PATCH,CRP,300,'clubs2'),x=await getRemote(PATCH,PP,300,'ui8'),y=await getRemote(COMP_PATCH,LP,300,'comp3'),z=await getRemote(THEME_PATCH,TP,300,'mutheme3'),tr=await getRemote(THEME_REGISTRY_PATCH,TRP,300,'themes7'),u=await getRemote(TOP_LAYOUT_PATCH,TLP,300,'toplayout2'),i=await getRemote(IDENTITY_PATCH,IP,300,'identity3'),e=await getRemote(EDGE_SAFE_PATCH,EP,300,'edgesafe1'),q=await getRemote(LIVE_CONTEXT_PATCH,LCP,250,'livectx1'),r=await getRemote(RESILIENCE_PATCH,RP,300,'resilience2');
...
/* Compatibility padding: keep this launcher above 1000 bytes because older installed recovery loaders reject downloaded runtime files shorter than 1000 bytes. Club Pulse injects declarative club registry v2 before core parameter resolution. Shared theme registry v8 fixes the outer shell to a common high-contrast dark navy for every themed club. Edge-safe identity v1 removes diagonal decoration from the information plane and replaces it with narrow vertical color rails at the far left/right of the inner match card; the central score/text/logo/date zone stays clean. Barcelona is pushed further toward a warm deep-purple identity, Real Madrid keeps a pearl-white inner card with blue/gold edge rails, and Manchester United keeps deep red/black/gold. Crest treatment remains transparent with no circular plate. Competition identity colors and logos remain shared across clubs. */
