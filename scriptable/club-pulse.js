...
/* Compatibility padding: keep this launcher above 1000 bytes because older installed recovery loaders reject downloaded runtime files shorter than 1000 bytes. Club Pulse injects declarative club registry v2 before core parameter resolution. Shared theme registry v7.2 keeps restrained dark club shells but strengthens the two diagonal club-color streaks so they remain visible at real iPhone widget scale. Small uses slightly broader streaks than Medium. Manchester United uses deep red plus restrained gold; Barcelona uses garnet plus deep blue; Real Madrid uses royal blue plus restrained gold. The streaks remain confined to the outer shell and do not cross the primary match information. Crest treatment remains transparent with no circular plate. The legacy Manchester United patch remains loaded only for its established match-state rendering and POST form-sync behavior. New clubs should normally be added through club registry plus shared theme tokens rather than cloned renderer files. Competition identity colors and logos remain shared across clubs. */