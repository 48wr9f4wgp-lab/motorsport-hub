...
let c=await core(),cr=await getRemote(CLUB_REGISTRY_PATCH,CRP,300,'clubs2'),x=await getRemote(PATCH,PP,300,'ui8'),y=await getRemote(COMP_PATCH,LP,300,'comp3'),z=await getRemote(THEME_PATCH,TP,300,'mutheme3'),tr=await getRemote(THEME_REGISTRY_PATCH,TRP,300,'themes7'),u=await getRemote(TOP_LAYOUT_PATCH,TLP,300,'toplayout2'),i=await getRemote(IDENTITY_PATCH,IP,300,'identity3'),q=await getRemote(LIVE_CONTEXT_PATCH,LCP,250,'livectx1'),r=await getRemote(RESILIENCE_PATCH,RP,300,'resilience2');
...
/* Compatibility padding: keep this launcher above 1000 bytes because older installed recovery loaders reject downloaded runtime files shorter than 1000 bytes. Club Pulse injects declarative club registry v2 before core parameter resolution. Shared theme registry v8 fixes the outer shell to a common high-contrast dark navy for every themed club, removing club-color streaks from the header/rank/update readability zone. Club identity now lives in the inner match surface plus two edge-safe streaks: Manchester United stays deep red/black/gold; Barcelona uses a stronger warm purple with garnet and restrained blue; Real Madrid keeps a pearl-white match surface with royal-blue and restrained gold. The shared footer rail is also dark and neutral, leaving only tiny accent cues club-specific. Crest treatment remains transparent with no circular plate. Competition identity colors and logos remain shared across clubs. */
