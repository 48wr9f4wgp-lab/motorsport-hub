...
let c=await core(),cr=await getRemote(CLUB_REGISTRY_PATCH,CRP,300,'clubs2'),x=await getRemote(PATCH,PP,300,'ui8'),y=await getRemote(COMP_PATCH,LP,300,'comp3'),z=await getRemote(THEME_PATCH,TP,300,'mutheme3'),tr=await getRemote(THEME_REGISTRY_PATCH,TRP,300,'themes3'),u=await getRemote(TOP_LAYOUT_PATCH,TLP,300,'toplayout2'),q=await getRemote(LIVE_CONTEXT_PATCH,LCP,250,'livectx1'),r=await getRemote(RESILIENCE_PATCH,RP,300,'resilience2');
...
/* Compatibility padding: keep this launcher above 1000 bytes because older installed recovery loaders reject downloaded runtime files shorter than 1000 bytes. Club Pulse injects declarative club registry v2 before core parameter resolution. Shared theme registry v3 now covers Manchester United, Barcelona, and Real Madrid with distinct club identities while reusing the same theme-driven composition. Manchester United uses saturated club red, near-black, and restrained crest gold; Barcelona uses Blaugrana garnet / deep blue / gold; Real Madrid uses pearl white / royal blue / gold. The legacy Manchester United patch remains loaded only for its established match-state rendering and POST form-sync behavior, while shared theme v3 owns common color/background/badge/footer identity. New clubs should normally be added through club registry plus shared theme tokens rather than cloned renderer files. Competition identity colors and logos remain shared across clubs. */