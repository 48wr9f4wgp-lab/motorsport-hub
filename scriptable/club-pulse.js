...
let c=await core(),cr=await getRemote(CLUB_REGISTRY_PATCH,CRP,300,'clubs2'),x=await getRemote(PATCH,PP,300,'ui8'),y=await getRemote(COMP_PATCH,LP,300,'comp3'),z=await getRemote(THEME_PATCH,TP,300,'mutheme3'),tr=await getRemote(THEME_REGISTRY_PATCH,TRP,300,'themes7'),u=await getRemote(TOP_LAYOUT_PATCH,TLP,300,'toplayout2'),i=await getRemote(IDENTITY_PATCH,IP,300,'identity1'),q=await getRemote(LIVE_CONTEXT_PATCH,LCP,250,'livectx1'),r=await getRemote(RESILIENCE_PATCH,RP,300,'resilience2');
...
/* Compatibility padding: keep this launcher above 1000 bytes because older installed recovery loaders reject downloaded runtime files shorter than 1000 bytes. Club Pulse injects declarative club registry v2 before core parameter resolution. Shared theme registry v7.2 keeps the established dark premium shell and strengthened diagonal club-color streaks. Identity override v1 then specializes Real Madrid and Barcelona without cloning the renderer: Real Madrid uses a pearl-white outer shell with royal-blue and restrained gold streaks while keeping the match panel dark navy; Barcelona uses a deep-purple outer shell with purple/blue streaks and a navy-plum match surface. Crest treatment remains transparent with no circular plate. Manchester United remains black/deep-red/gold. The legacy Manchester United patch remains loaded only for its established match-state rendering and POST form-sync behavior. New clubs should normally be added through club registry plus shared theme tokens rather than cloned renderer files. Competition identity colors and logos remain shared across clubs. */