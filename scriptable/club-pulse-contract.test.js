...
check('launcher loads theme registry v5 cache',has(files.launcher,'ClubPulseThemeRegistryPatch_v5.js')&&has(files.launcher,"'themes5'"));
...
check('Real theme is navy white gold v5',has(files.themes,"bg:['#020712','#061A3A','#0A3475','#0B3D91','#05162F']")&&has(files.themes,"next:['#0F4FA7','#0B377B','#071F48','#030B18']")&&has(files.themes,"text:'#FAFCFF'")&&has(files.themes,"accent:'#FEBE10'"));
...
check('Real keeps pearl crest surface',has(files.themes,"t.key==='realmadrid'?'#FFFFFF':t.accent")&&has(files.themes,"t.key==='realmadrid'?1.15:.8"));
