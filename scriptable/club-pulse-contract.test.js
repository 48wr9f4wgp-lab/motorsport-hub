...
check('launcher loads theme registry v10 cache',has(files.launcher,'ClubPulseThemeRegistryPatch_v10.js')&&has(files.launcher,"'themes10'"));
check('launcher loads identity v4 cache',has(files.launcher,'ClubPulseIdentityColorPatch_v4.js')&&has(files.launcher,"'identity4'"));
...
check('inner cards use simple gradient',has(files.themes,'function cpSimpleCardGradient')&&has(files.themes,'return cpSimpleCardGradient(t,mode)'));
...
check('Man U uses vivid official-style red with black and gold',has(files.themes,"cardSurface:'#3A070B'")&&has(files.themes,"cardPanel:'#B5121B'")&&has(files.themes,"cardGlow:'#DA291C'")&&has(files.themes,"accent:'#D6B45A'"));
check('Barcelona uses strong warm purple',has(files.themes,"cardSurface:'#150A2E'")&&has(files.identity,"barca.cardPanel='#30145B'")&&has(files.identity,"barca.cardGlow='#3F1C70'"));
