...
check('small presentation defines stronger team-name hierarchy',small.includes('const CP_SP_TYPO=')&&small.includes('headerName:9.0')&&small.includes('team:9.6')&&small.includes('meta:8.8')&&small.includes('footer:7.6'));
check('generic team labels enforce readability scale floor',small.includes('teamMinScale:.86')&&small.includes('nm.minimumScaleFactor=CP_SP_TYPO.teamMinScale'));
check('generic team labels stay single-line clipping safe',small.includes('nm.lineLimit=1')&&small.includes('teamWidth:52'));
...
check('all small teams share rebalanced canonical block geometry',small.includes('teamWidth:52')&&small.includes('scoreWidth:36')&&small.includes('logo:40'));
check('center score column stays clipping-safe after rebalance',small.includes('scoreWidth:36')&&small.includes('scoreMinScale:.78')&&small.includes('sb.size=new Size(CP_SP_TYPO.scoreWidth,22)'));
...
console.log('\nClub Pulse canonical Small Presentation System v6 QA PASSED');