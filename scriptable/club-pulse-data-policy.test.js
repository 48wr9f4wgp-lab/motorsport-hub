...
function makeContext({now=Date.now(),mode='NEXT',kickoffMs=now+72*3600e3,cachedAge=10*60e3,nextCache=null,quotaCount=0,liveRows=null}={}){
  ...
  const store={
    '/cache/data_manutd.json':cached,
    '/cache/standings_pl.json':{fetchedAt:now-5*60e3,payload:standings},
    '/cache/api_football_quota.json':{day:'2026-08-31',count:quotaCount}
  };
  ...
  const ctx={
    ...
    quotaPath:()=>'/cache/api_football_quota.json',
    fmt:()=> '2026-08-31',
    NEXT_TTL:6*3600e3,
    TTL_LIVE:3*60e3,
    ...
    getLiveToken:()=> 'live-token',
    shouldCheckLive:()=>true,
    resolveLiveTeamId:async()=>{resolveCalls++;return 33},
...
(async()=>{
  ...
  const now=Date.now();
  c=makeContext({now,nextCache:{fetchedAt:now-10*3600e3,match:null}});
  ...
  c=makeContext({now,kickoffMs:now+10*60e3,liveRows:[]});
  out=await c.applyLiveOverlay(c.__state.cached);
  out=await c.applyLiveOverlay(c.__state.cached);
  ...
  const small=c.buildMatchSmall(null,staleNext,{});
