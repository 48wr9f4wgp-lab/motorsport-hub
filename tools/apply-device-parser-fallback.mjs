...
const wecBlock=`function makerName(raw){return String(raw||'').replace(/\\bImage\\b/gi,' ').replace(/\\s+/g,' ').trim().toUpperCase()}
...
function wecPlainRows(plain){
 ...
 const out=[];for(let i=0;i<marks.length;i++){const q=marks[i],chunk=section.slice(q.end,i+1<marks.length?marks[i+1].start:section.length),nums=(chunk.match(/-?\\d+(?:\\.\\d+)?/g)||[]).map(Number).filter(Number.isFinite),pts=nums.length?nums[nums.length-1]:NaN;if(!(q.pos>=1&&q.pos<=30)||!META[q.maker]||!isFinite(pts))continue;const meta=META[q.maker];out.push({pos:q.pos,name:q.maker,points:\\`${'${pts}'} pts\\`,maker:q.maker,machine:meta[0],team:meta[1]})}return out
}
async function update(d){
 ...
 for(const c of rows(h)){if(c.length<3)continue;const p=num(c[0]),maker=makerName(c[1]),pts=num(c[c.length-1]);if(!(p>=1&&p<=30)||!META[maker]||!isFinite(pts))continue;const m=META[maker];a.push({pos:p,name:maker,points:\\`${'${pts}'} pts\\`,maker,machine:m[0],team:m[1]})}
...
const sgtBlock=`function fallbackName(raw){const s=String(raw||'').replace(/　/g,' ').replace(/\\s+/g,' ').trim();return s||'GT500 DRIVER'}
...
async function update(d){
 ...
 let a=[];if(totalIndex>=3){for(const c of tableRows){if(c.length<=totalIndex)continue;const p=num(c[0]),no=String(c[1]||'').match(/\\d+/)?.[0]||'',pts=num(c[totalIndex]);if(!(p>=1&&p<=30)||!GT500_NOS.has(no)||!isFinite(pts))continue;const meta=META[no]||{name:fallbackName(c[2]),maker:'',machine:'GT500',team:\\`No.${'${no}'}\\`};a.push({pos:p,no,name:meta.name||fallbackName(c[2]),points:\\`${'${pts}'} pts\\`,maker:meta.maker||'',machine:meta.machine||'GT500',team:meta.team||\\`No.${'${no}'}\\`})}}
...
console.log('Applied device parser fallback patch.');
