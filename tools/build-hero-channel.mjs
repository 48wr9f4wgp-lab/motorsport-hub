...
function pruneAssets(category,entry){const dir=path.join(outputDir,'assets',category);if(!fs.existsSync(dir))return;const refs=new Set();for(const a of [coreAsset(entry),...(entry.pool||[])])for(const f of ['small','medium','large']){const u=a?.images?.[f]?.url;if(u)refs.add(path.basename(new URL(u).pathname))}for(const n of fs.readdirSync(dir))if(/\.jpg$/i.test(n)&&!refs.has(n))fs.rmSync(path.join(dir,n),{force:true});}
