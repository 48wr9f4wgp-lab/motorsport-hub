...
function dedupePool(items){const seen=new Set(),out=[];for(const x of items){if(!x?.assetId||seen.has(x.assetId)||[...seen].includes(x.sourcePage))continue;seen.add(x.assetId);seen.add(x.sourcePage);out.push(x)}return out}
...
for(const dir of artifactDirs()){
 ...
 if(selected){const live=liveFromPool(prev,selected,pool,prev?.recentAssetIds||[],mode);next.categories[category]=live;promoted.push(category);promotionModes[category]=mode;updatedCategories.push(category)}
