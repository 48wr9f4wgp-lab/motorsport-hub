...
(async()=>{
  ...
  const lkgPath=fm.joinPath(dir,'motorsport-hub-router-v5-lkg.js');
  const quarantinePath=fm.joinPath(dir,'motorsport-hub-router-v5-quarantine.js');

  const syntaxOK=s=>{
    try{new Function(String(s||''));return true}catch(_){return false}
  };
  ...
  const quarantine=code=>{
    try{if(typeof code==='string'&&code.length)fm.writeString(quarantinePath,code)}catch(_){}
  ...
  const fail=async msg=>{
    ...
    b.font=Font.systemFont(10);b.textColor=new Color('#FFB84D');b.lineLimit=4;
  ...
  try{
    ...
    r.headers={
      ...
      'Pragma':'no-cache',
      'Expires':'0',
      'User-Agent':'MotorsportHubLoader/5'
    };
    candidate=await r.loadString();
  ...
  await fail('検証済みRouterを取得できません。古いRouterへは戻さず、通信回復後に再試行します。');
})();
