// Motorsport Hub v9.3.6-hardening — module router / F1 + FDJ + MotoGP flatten pilots
...
// H4: F1, FDJ and MotoGP use completed dedicated modules; remaining legacy categories retain the bridge circuit breaker.
// MH_ROUTER_SCHEMA=5
...
const isF1=selected==='F1',isFDJ=selected==='FDJ',isMoto=selected==='MOTOGP',isQA=selected==='QA',isD1=selected==='D1GP',isSF=selected==='SUPERFORMULA',isINDY=selected==='INDYCAR',isNASCAR=selected==='NASCAR',isGTWC=selected==='GTWCEU';
const isExpansion=isSF||isINDY||isNASCAR||isGTWC;
const file=isF1?'f1-widget-flat-v1000.js':isFDJ?'fdj-widget-flat-v1000.js':isMoto?'motogp-widget-flat-v1000.js':isQA?'motorsport-diagnostics-v890.js':isD1?'d1gp-reliability-v890.js':isSF?'superformula-widget.js':isINDY?'indycar-widget.js':isNASCAR?'nascar-widget.js':isGTWC?'gtwc-europe-widget.js':'motorsport-reliability-v896.js';
const key=isF1?'f1-flat-v1000':isFDJ?'fdj-flat-v1000':isMoto?'motogp-flat-v1000':isQA?'diagnostics-v890':isD1?'d1gp-v890':isSF?'superformula-v900':isINDY?'indycar-v910':isNASCAR?'nascar-v920':isGTWC?'gtwceu-v930':'reliability-v896';
...
const valid=s=>typeof s==='string'&&s.includes('Motorsport Hub')&&s.includes('Script.complete()')&&(isF1?s.includes('flattened F1 pilot module'):isFDJ?s.includes('flattened Formula Drift Japan module'):isMoto?s.includes('flattened MotoGP module'):isQA?s.includes('QA diagnostics'):isD1?s.includes('D1GP reliability wrapper'):isSF?s.includes('SUPER FORMULA module'):isINDY?s.includes('INDYCAR module'):isNASCAR?s.includes('NASCAR Cup Series module'):isGTWC?s.includes('GT World Challenge Europe module'):s.includes('Reliability Pass'));
...
let code='';
if(globalThis.__MH_REMOTE_OFFLINE!==true){try{const r=new Request(`${URL}?v=936&t=${Date.now()}-${Math.random()}`);r.timeoutInterval=15;r.headers={'Cache-Control':'no-cache, no-store, max-age=0, must-revalidate','Pragma':'no-cache','Expires':'0','User-Agent':'MotorsportHubRouter/9.3.6-hardening'};code=await r.loadString();if(!valid(code))throw Error('invalid module');fm.writeString(cache,code)}catch(e){globalThis.__MH_REMOTE_OFFLINE=true}}
...
if(!isF1&&!isFDJ&&!isMoto&&!isQA&&!isD1&&!isExpansion)globalThis.__MH_UNIVERSAL_PARAMETER=selected;
