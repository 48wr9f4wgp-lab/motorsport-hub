...
const isF1=selected==='F1',isQA=selected==='QA',isD1=selected==='D1GP',isSF=selected==='SUPERFORMULA',isINDY=selected==='INDYCAR',isNASCAR=selected==='NASCAR',isGTWC=selected==='GTWCEU';
const isExpansion=isSF||isINDY||isNASCAR||isGTWC;
const file=isF1?'f1-widget-flat-v1000.js':isQA?'motorsport-diagnostics-v890.js':isD1?'d1gp-reliability-v890.js':isSF?'superformula-widget.js':isINDY?'indycar-widget.js':isNASCAR?'nascar-widget.js':isGTWC?'gtwc-europe-widget.js':'motorsport-reliability-v896.js';
const key=isF1?'f1-flat-v1000':isQA?'diagnostics-v890':isD1?'d1gp-v890':isSF?'superformula-v900':isINDY?'indycar-v910':isNASCAR?'nascar-v920':isGTWC?'gtwceu-v930':'reliability-v896';
...
const valid=s=>typeof s==='string'&&s.includes('Motorsport Hub')&&s.includes('Script.complete()')&&(isF1?s.includes('flattened F1 pilot module'):isQA?s.includes('QA diagnostics'):isD1?s.includes('D1GP reliability wrapper'):isSF?s.includes('SUPER FORMULA module'):isINDY?s.includes('INDYCAR module'):isNASCAR?s.includes('NASCAR Cup Series module'):isGTWC?s.includes('GT World Challenge Europe module'):s.includes('Reliability Pass'));
...
if(!isF1&&!isQA&&!isD1&&!isExpansion)globalThis.__MH_UNIVERSAL_PARAMETER=selected;
