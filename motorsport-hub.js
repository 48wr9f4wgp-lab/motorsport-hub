// Motorsport Hub v8.7.1 — module router
// Final Visual Polish: universal readability retained, FDJ action hero restored, WRC PTS contrast strengthened.
...
const file='motorsport-universal-v871.js';
const URL=`https://raw.githubusercontent.com/48wr9f4wgp-lab/motorsport-hub/main/${file}`;
const fm=FileManager.local(),cache=fm.joinPath(fm.documentsDirectory(),'motorsport-hub-module-universal-v871.js');
const valid=s=>typeof s==='string'&&s.includes('Motorsport Hub v8.7.1')&&s.includes('Final Visual Polish')&&s.includes('Script.complete()');
...
async function fail(){
  ...
  w.addSpacer(6);const b=w.addText('v8.7.1共通表示モジュールを取得できません。旧版へ戻さず再試行します。');b.font=Font.systemFont(10);b.textColor=new Color('#FFB84D');b.lineLimit=3;
...
try{
  ...
  r.headers={'Cache-Control':'no-cache, no-store, max-age=0, must-revalidate','Pragma':'no-cache','Expires':'0','User-Agent':'MotorsportHubRouter/8.7.1'};
