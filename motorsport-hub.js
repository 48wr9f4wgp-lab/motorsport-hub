// Motorsport Hub v9.3.2-hardening — module router / explicit parameter validation
...
let selected=norm(rawParameter);selected=aliases[selected]||selected;
...
if(!config.runsInWidget&&!params.includes(selected)){
  const a=new Alert();a.title='Motorsport Hub';a.message='プレビューするカテゴリ';labels.forEach(x=>a.addAction(x));a.addCancelAction('キャンセル');
...
const isQA=selected==='QA',isD1=selected==='D1GP',isSF=selected==='SUPERFORMULA',isINDY=selected==='INDYCAR',isNASCAR=selected==='NASCAR',isGTWC=selected==='GTWCEU';
...
const valid=s=>typeof s==='string'&&s.includes('Motorsport Hub')&&s.includes('Script.complete()')&&(isQA?s.includes('QA diagnostics'):isD1?s.includes('D1GP reliability wrapper'):isSF?s.includes('SUPER FORMULA module'):isINDY?s.includes('INDYCAR module'):isNASCAR?s.includes('NASCAR Cup Series module'):isGTWC?s.includes('GT World Challenge Europe module'):s.includes('Reliability Pass'));

function replaceExact(s,needle,replacement,expected,label){
  const parts=String(s).split(needle),hits=parts.length-1;
  if(hits!==expected)throw Error(`HARDENING_PATCH_MISMATCH:${label}:${hits}/${expected}`);
  return parts.join(replacement);
}
function hardenExpansionLifecycle(src){
  if(!isExpansion)return src;
  ...
  let out=replaceExact(src,oldNext,newNext,1,'nextEvent');
  out=replaceExact(out,oldCountdown,newCountdown,1,'countdown');
  out=replaceExact(out,"T(top,'次戦',","T(top,d.lifecycle==='SEASON_ENDED'?'シーズン終了':'次戦',",2,'header');
  if(!out.includes("lifecycle:'SEASON_ENDED'")||!out.includes("now>=s&&now<e")||!out.includes("?'シーズン終了':'次戦'"))throw Error('HARDENING_PATCH_POSTCONDITION');
  return out;
}
...
if(globalThis.__MH_REMOTE_OFFLINE!==true){
  try{
    const r=new Request(`${URL}?v=932&t=${Date.now()}-${Math.random()}`);r.timeoutInterval=15;
    r.headers={'Cache-Control':'no-cache, no-store, max-age=0, must-revalidate','Pragma':'no-cache','Expires':'0','User-Agent':'MotorsportHubRouter/9.3.2-hardening'};
...
globalThis.__MH_ROUTER_BOOT_OK=true;
if(!isQA&&!isD1&&!isExpansion)globalThis.__MH_UNIVERSAL_PARAMETER=selected;

const originalLoadString=Request.prototype.loadString;
Request.prototype.loadString=async function(){
  const u=String(this?.url||''),repoRaw=u.includes('raw.githubusercontent.com/48wr9f4wgp-lab/motorsport-hub/');
  if(repoRaw&&globalThis.__MH_REMOTE_OFFLINE===true)throw Error('MH_REPO_RAW_CIRCUIT_OPEN');
  try{return await originalLoadString.call(this)}catch(e){if(repoRaw)globalThis.__MH_REMOTE_OFFLINE=true;throw e}
};
