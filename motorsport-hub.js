// Motorsport Hub v9.4.3-hardening — direct category module router
...
if(!params.includes(selected)||!ROUTES[selected]){
...
const cache=fm.joinPath(fm.documentsDirectory(),`motorsport-hub-module-${route.key}.js`);
...
if(globalThis.__MH_REMOTE_OFFLINE!==true){
 try{
  const r=new Request(`${URL}?v=943&t=${Date.now()}-${Math.random()}`);r.timeoutInterval=15;
  r.headers={'Cache-Control':'no-cache, no-store, max-age=0, must-revalidate','Pragma':'no-cache','Expires':'0','User-Agent':'MotorsportHubRouter/9.4.3-hardening'};
...
if(!valid(code)){
 try{
  if(fm.fileExists(cache)){
   ...
   if(valid(c))code=c;else fm.remove(cache);
  }
 }catch(_){}
}
